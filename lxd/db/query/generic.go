...
func Create(ctx context.Context, tx *sql.Tx, c Creatable) (int64, error) {
	...
	if err != nil {
		if IsConflictErr(err) {
			return -1, api.StatusErrorf(http.StatusConflict, "Entry in table %q already exists: %w", tableName, err)
		}
...
func Update(ctx context.Context, tx *sql.Tx, u Updatable) error {
	...
	if err != nil {
		if IsConflictErr(err) {
			return api.StatusErrorf(http.StatusConflict, "Entry in table %q already exists: %w", tableName, err)
		}
...
// Note that this function does not call DeleteOne. This is because the Referencable argument might
...
func DeleteOne[T TableNamer, PT interface {
	...
	*T
	...
	tableName := (*new(T)).TableName()
	if n < 1 {
		return api.StatusErrorf(http.StatusNotFound, "Failed deleting a single row from %q", tableName)
	} else if n > 1 {
