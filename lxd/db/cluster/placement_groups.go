...
// PlacementGroupConfig returns a [query.Configurator] for placement groups.
func PlacementGroupConfig() query.Configurator {
	return query.Configurator{
		EntityTable: "placement_groups",
		ConfigTable: "placement_groups_config",
		ForeignKey:  "placement_group_id",
	}
}
...
func GetPlacementGroupConfigs(ctx context.Context, tx *sql.Tx, projectName *string) (map[int64]map[string]string, error) {
	...
	return PlacementGroupConfig().Get(ctx, tx, clause, args...)
}
