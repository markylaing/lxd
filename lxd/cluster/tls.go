...
func tlsCheckCert(r *http.Request, networkCert *shared.CertInfo, serverCert *shared.CertInfo, cache *identity.Cache) bool {
	...
	for _, i := range r.TLS.PeerCertificates {
		...
		trusted, _ := util.CheckTrustState(*i, map[string]x509.Certificate{serverCert.Fingerprint(): *trustedServerCert})
		if trusted {
			return true
		}

		// Check the trusted server certficates list provided.
		trusted, _ = util.CheckTrustState(*i, cache.X509Certificates(api.IdentityTypeCertificateServer))
		if trusted {
			return true
		}
