...
func createIdentityTLSTrusted(ctx context.Context, s *state.State, peerCertificates []*x509.Certificate, networkCert *shared.CertInfo, req api.IdentitiesTLSPost, notify identityNotificationFunc) response.Response {
	...
	err = s.DB.Cluster.Transaction(ctx, func(ctx context.Context, tx *db.ClusterTx) error {
		identityID, err := dbCluster.CreateTLSIdentity(ctx, tx.Tx(), req.Name, api.IdentityTypeCertificateClient, *x509Cert)
		if err != nil {
...
func validateIdentityCert(networkCert *shared.CertInfo, cert string) (*x509.Certificate, error) {
	...
	if err != nil {
		return nil, api.StatusErrorf(http.StatusBadRequest, "Failed parsing certificate: %w", err)
	}
