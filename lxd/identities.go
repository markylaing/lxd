...
func updateIdentityCache(d *Daemon) {
	...
	serverCertificates := make(map[string]*x509.Certificate)
	...
	for _, id := range identities {
		...
		if identityType.AuthenticationMethod() == api.AuthenticationMethodTLS {
			...
			if id.Type == api.IdentityTypeCertificateServer {
				...
				serverCertificates[id.Identifier] = cert
			} else {
				clientCertificates[id.Identifier] = cert
			}
	...
	err = d.identityCache.ReplaceAll(serverCertificates, clientCertificates, secrets)
	if err != nil {
...
func updateIdentityCacheFromLocal(d *Daemon) error {
	...
	serverCerts := make(map[string]*x509.Certificate, len(localServerCerts))
	...
	err = d.identityCache.ReplaceAll(serverCerts, nil, nil)
	if err != nil {
