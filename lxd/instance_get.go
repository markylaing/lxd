...
func instanceGet(d *Daemon, r *http.Request) response.Response {
	...
	if len(withEntitlements) > 0 {
		err = reportEntitlements(r.Context(), s.Authorizer, entity.TypeInstance, withEntitlements, map[int]auth.EntitlementReporter{c.ID(): state.(auth.EntitlementReporter)})
		if err != nil {
			return response.SmartError(err)
		}
	}
