...
func (c *cmdForknet) RunDetach(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		...
		_, altErr := shared.RunCommandContext(ctx, "ip", "link", "property", "del", "dev", ifName, "altname", hostName)
		if altErr == nil {
			err = link.SetName(hostName)
		}
	...
	if shared.PathExists(phyPath) {
		...
		_, err = shared.RunCommandContext(ctx, "iw", "phy", strings.TrimSpace(string(phyName)), "set", "netns", lxdPID)
		if err != nil {
			return err
		}
	} else {
