...
func placementGroupsPost(d *Daemon, r *http.Request) response.Response {
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		err = cluster.PlacementGroupConfig().Set(ctx, tx.Tx(), id, req.Config)
		if err != nil {
			return err
		}
...
func placementGroupGet(d *Daemon, r *http.Request) response.Response {
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		configs, err := cluster.PlacementGroupConfig().GetByEntityIDs(ctx, tx.Tx(), dbGroup.Row.ID)
		if err != nil {
...
func placementGroupPut(d *Daemon, r *http.Request) response.Response {
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		config, err = cluster.PlacementGroupConfig().GetByEntityID(ctx, tx.Tx(), placementGroup.Row.ID)
		if err != nil {
			return fmt.Errorf("Failed getting placement group config: %w", err)
		}
	...
	err = s.DB.Cluster.Transaction(r.Context(), func(ctx context.Context, tx *db.ClusterTx) error {
		...
		err = cluster.PlacementGroupConfig().Set(ctx, tx.Tx(), updatedPlacementGroup.Row.ID, updatedConfig)
		if err != nil {
			return err
		}
